...
function checkAll( n, fldName ) {
	...
	if (c) {
		$('#boxchecked').get(0).value = n2;
		if(n2>0)
		{
			if($.browser.msie)
			{
				$('div.bottom-controls').show();
			}
			else
			{
				$('div.bottom-controls').slideDown('normal');
			}
		}
	} else {
		$('#boxchecked').get(0).value = 0;
		if(n2>0)
		{
			if($.browser.msie)
			{
				$('div.bottom-controls').hide();
			}
			else
			{
				$('div.bottom-controls').slideUp('normal');
			}
		}
	}
}
...
function isChecked(isitchecked){
	if (isitchecked == true){
		$('#boxchecked').get(0).value++;
		if($('#boxchecked').get(0).value == 1)
		{
			if($.browser.msie)
			{
				$('div.bottom-controls').show();
